...
class LungDatasetExp3(Dataset):
    def __init__(self, label_file, base_dir, task, split="train", transform=None, train_prop=1.0, df=None, data=None,
        ...
        if data is None:
            try:
                file = base_dir + '/data/processed/' + 'disease' + '_1.0.h5'
                file = h5.File(file, 'r')
...
class LungDataset(Dataset):
    def __init__(self, label_file, base_dir, task, split="train", transform=None, train_prop=1.0, df=None, data=None):
        ...
        if data is None:
            try:
                file = os.path.join(base_dir, 'data', 'processed', 'disease_1.0.h5')
                file = h5.File(file, 'r')
                print(f'file = {file}') # for ERROR check
                data = file[split][df.index.tolist()]
            except:
                raise Exception("Data not found")
        self.data = data
    ...
    def get_split(self, df, split_file_path, train_prop=1.0):
        # Takes in a DataFrame and a path to a file of only Ints denoting Patient ID.
        # Returns a DataFrame of only samples with Patient IDs contained in the split.
        # C:\Users\user\DeepDaiv\selfsupervised-lungandheartsounds\data\splits\val.txt
        print(f'split_file_path in get_split = {split_file_path}')
        IDs = set()
        with open(split_file_path, "r") as f:
            IDs = set([line.strip() for line in f])
            print(IDs)
            df = df[df.ID.isin(IDs)]
            print(f'1111111 df = {df}')
            df = df.reset_index()
            print(f'2222222 df = {df}')
            cycles = set(list(df.cycle))
            print(f'3333333333 df = {df}')
            cycles = set(random.sample(cycles, int(train_prop * len(cycles))))
            print(f'4444444444 df = {df}')
