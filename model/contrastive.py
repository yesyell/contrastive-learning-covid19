...
class ContrastiveLearner(object):
    ...
    def pre_train(self, log_file, task, label_file, augment=None, learning_rate=0., restore=False):
        test_dataset = get_dataset(task, label_file, os.path.join(base_dir), split="val")
        ...
        if self.exp in [2, 4, 5, 6]:
            self.batch_size = 1
        # if self.batch_size == 0: # for ERROR solving
        #     self.batch_size = 1
...
def pretrain_(epochs, task, base_dir, log_dir, augment, train_prop=1, exp=None, restore=False):
    ...
    label_file = os.path.join(base_dir, "data", "processed", "{}_labels.csv".format(task))
...
if __name__ == "__main__":
    ...
    cur_dir = os.path.join(os.getcwd(), args.data)
